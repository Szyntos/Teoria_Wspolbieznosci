...
public class Main2 {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        Producer differentProducer = new Producer(0, m, buffer, seedRNG.randomInt(0, 10000));
        consumers[0] = new Consumer(0, m, buffer, seedRNG.randomInt(0, 10000));
        producers[0] = differentProducer;
...
//            sleep(30000);
    ...
    public static void printAverageTimes(int bufferSize, int threadsNumber) throws IOException {
        ...
        for (int i = 1; i < bufferSize; i++){
            writer.write("takefourcond,"+bufferSize+","+threadsNumber+","+i+","+getMeans.get(i)+"\n");
            System.out.println("takefourcond,"+bufferSize+","+threadsNumber+","+i+","+getMeans.get(i));
        }

        for (int i = 1; i < bufferSize; i++){
            writer.write("putfourcond,"+bufferSize+","+threadsNumber+","+i+","+putMeans.get(i)+"\n");
            System.out.println("putfourcond,"+bufferSize+","+threadsNumber+","+i+","+putMeans.get(i));
        }
    ...
    public static void printWaitingLoops(int bufferSize, int threadsNumber) throws IOException {
        ...
        for (int i = 0; i < threadsNumber; i++){
            if (producers[i] != null){
                ...
                writer.write("producerfourcond,"+bufferSize+","+threadsNumber+","+i+","+producers[i].made+"\n");
            }
        }
        System.out.println("==========================================");
        for (int i = 0; i < threadsNumber; i++){
            if (consumers[i] != null) {
                ...
                writer.write("consumerfourcond,"+bufferSize+","+threadsNumber+","+i+","+consumers[i].taken+"\n");
            }
        }
        System.out.println("**** **** **** ****  ****  **** **** **** ****");
